...
{
	//BOOST_ASSERT(typeindex(T1) != typeindex(T2));
}
...
bool DataBuffer<T1, T2>::Initial(char* setting, long settingLen)
{
	...
	{
		OutputDebugStringEx("DataBuffer::Initial().", LogLevel::debugLevel);
		return false;
	}
	return rtVal;
}
...
void DataBuffer<T1, T2>::Unitial()
{
	...
	{
		OutputDebugStringEx("DataBuffer::Unitial().", LogLevel::debugLevel);
		return;
	}
	INode::Unitial();
}
...
bool DataBuffer<T1, T2>::Set(char * setting, long settingLen)
{
	...
	{
		OutputDebugStringEx("DataBuffer::Set().", LogLevel::debugLevel);
		return false;
	}
...
bool DataBuffer<T1, T2>::Get(char * setting, long settingLen)
{
	...
	{
		OutputDebugStringEx("DataBuffer::Get().", LogLevel::debugLevel);
		return false;
	}
...
template <typename T1, typename T2 = T1>
bool DataBuffer<T1,T2>::Click(bool isSync)
{
	return false;
}